...
export default function Tables() {
  ...
  // useEffect(()=>{
  //  dispatch(getTables(idResto,token))
  // },[dispatch, idResto, token]);
      ...
      <div className="container justify-center flex flex-col w-screen">
        <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          ...
          <table>
            ...
            <tbody>
              {currentTables.map((tables) => (
                <tr key={tables.id}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {/* <Link to={`./${tables.tableId}`} className="link"> */}
                      <div className="text-sm text-gray-900">{tables.tableId}</div>
                    {/* </Link> */}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                    {tables.currentOrder.time ? (
                      <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">                        
                        {Moment(tables.currentOrder.time).format("HH:mm:ss")}
                      </span>
                    ) : (
                      <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                      </span>
